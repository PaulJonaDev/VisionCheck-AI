...
{
  const json = await request('/upload', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ imageUri, meta }) })
...
export function explain(predictions: PatternPrediction[]) {
  ...
  const lines = predictions.map(p => {
    ...
    const name = p.pattern === 'enrojecimiento' ? 'Enrojecimiento' : p.pattern === 'opacidad' ? 'Opacidad' : p.pattern === 'reflejo_irregular' ? 'Reflejo irregular' : p.pattern === 'inflamacion' ? 'Posible inflamación' : 'Fatiga ocular'
  ...
  predictions.forEach(p => {
    if (p.confidence >= 0.3) {
      ...
      if (hs) hs.slice(0, 2).forEach(h => { if (!hypotheses.includes(h)) hypotheses.push(h) })
    }
  })
  const exams: string[] = []
  predictions.forEach(p => {
    if (p.confidence >= 0.3) {
      ...
      if (es) es.forEach(e => { if (!exams.includes(e)) exams.push(e) })
    }
  })
  const recommendation = { callToAction: 'Se recomienda una evaluación con un profesional de la visión (optómetra u oftalmólogo).', exams }
  let lensScore = 0
  predictions.forEach(p => {
